...
async function chat(req: IncomingMessage, res: ServerResponse) {
  ...
  let response: Response; try { response = await fetch(`${GROQ_API_BASE}/chat/completions`, { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: process.env.GROQ_MODEL?.trim() || DEFAULT_LLM_MODEL, temperature: 0.3, messages: [{ role: 'system', content: `${SYSTEM_PROMPT} Prefer language code ${language} for the response.` }, ...messages] }) }); } catch (error) { sendJson(res, 502, { error: 'The AI service could not be reached.' }); return; }
...
async function assessQuality(req: IncomingMessage, res: ServerResponse) {
  ...
  const metadata = JSON.stringify({ cropName, variety: typeof body.variety === 'string' ? body.variety.trim() : '', harvestDate: typeof body.harvestDate === 'string' ? body.harvestDate : '', quantityQuintals: body.quantityQuintals, storageCondition: typeof body.storageCondition === 'string' ? body.storageCondition : '', storageLocation: typeof body.storageLocation === 'string' ? body.storageLocation : '', handlingNotes: typeof body.handlingNotes === 'string' ? body.handlingNotes : '', imageCount: images.length });
  ...
  let response: Response; try { response = await fetch(`${GEMINI_API_BASE}/models/${process.env.GEMINI_VISION_MODEL?.trim() || 'gemini-2.5-flash'}:generateContent?key=${encodeURIComponent(apiKey)}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text: prompt }, ...imageParts] }], generationConfig: { temperature: 0.1, responseMimeType: 'application/json' } }) }); } catch (error) { logProviderError('quality assessment network request', 502, error instanceof Error ? error.message : 'Network error'); sendJson(res, 502, { error: 'The quality assessment service could not be reached.' }); return; }
  if (!response.ok) { logProviderError('quality assessment provider request', response.status, await readProviderError(response)); sendJson(res, 502, { error: 'The quality assessment service is temporarily unavailable.' }); return; }
  const providerPayload = await response.json(); const raw = textFromGemini(providerPayload); if (typeof raw !== 'string') { sendJson(res, 502, { error: 'The quality assessment service returned no result.' }); return; }
  let result: Record<string, unknown>; try { result = JSON.parse(cleanJsonText(raw)) as Record<string, unknown>; } catch { sendJson(res, 502, { error: 'The quality assessment service returned an unreadable result.' }); return; }
  const grade = result.grade; const score = Number(result.score); if ((grade !== 'A' && grade !== 'B' && grade !== 'C') || !Number.isFinite(score)) { sendJson(res, 502, { error: 'The quality assessment service returned an incomplete result.' }); return; }
  sendJson(res, 200, { grade, score: Math.max(0, Math.min(100, Math.round(score))), reasoning: typeof result.reasoning === 'string' ? result.reasoning : 'The assessment is based on the submitted images and crop information.', visibleObservations: Array.isArray(result.visibleObservations) ? result.visibleObservations.filter(item => typeof item === 'string').slice(0, 8) : [], freshnessAssessment: typeof result.freshnessAssessment === 'string' ? result.freshnessAssessment : 'Freshness could not be determined beyond the visible evidence.', recommendations: Array.isArray(result.recommendations) ? result.recommendations.filter(item => typeof item === 'string').slice(0, 8) : [], sellingRecommendation: typeof result.sellingRecommendation === 'string' ? result.sellingRecommendation : 'Review the matched buyer options before selling.', confidence: result.confidence === 'high' || result.confidence === 'medium' ? result.confidence : 'low' });
}

async function handleRequest(req: IncomingMessage, res: ServerResponse, next: () => void) {
  const path = req.url?.split('?')[0] || '';
  if (path === '/api/voice/status' && req.method === 'GET') { sendJson(res, 200, { configured: Boolean(getGroqKey()) }); return; }
  if (path === '/api/voice/transcribe' && req.method === 'POST') { await transcribe(req, res); return; }
  if (path === '/api/voice/speak' && req.method === 'POST') { await speak(req, res); return; }
  if (path === '/api/voice/chat' && req.method === 'POST') { await chat(req, res); return; }
  if (path === '/api/quality/assess' && req.method === 'POST') { await assessQuality(req, res); return; }
  next();
}

export function groqApiPlugin(): Plugin {
  return { name: 'kisanlink-groq-api', configureServer(server) { server.middlewares.use((req, res, next) => { void handleRequest(req, res, next); }); } };
}
